...
import '../viewmodel/featured_clinics_cubit.dart';
import '../viewmodel/featured_clinics_state.dart';

class FeaturedClinicsSection extends StatelessWidget {
  const FeaturedClinicsSection({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<FeaturedClinicsCubit>()..loadClinics(),
      child: const _FeaturedClinicsBody(),
    );
  }
}

class _FeaturedClinicsBody extends StatelessWidget {
  const _FeaturedClinicsBody();
  ...
  Widget build(BuildContext context) {
    ...
    return KeyedSubtree(
      ...
      child: BlocBuilder<FeaturedClinicsCubit, FeaturedClinicsState>(
        builder: (context, state) {
          if (state.status == FeaturedClinicsStatus.loading) {
            return const _FeaturedClinicsLoading();
          }

          if (state.status == FeaturedClinicsStatus.failure) {
            return Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 24),
              child: Text(
                state.message ?? 'home.featured_clinics.error'.tr(),
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                _SectionHeader(
                  title: 'home.featured_clinics.title'.tr(),
                  actionLabel: 'home.featured_clinics.see_more'.tr(),
                  onTap: () => _openSeeMore(context, state.clinics),
                ),
                ...
                SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      final hospital = state.clinics[index];
  ...
  void _openSeeMore(BuildContext context, List<HospitalModel> clinics) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => NavcareHospitalsPage(hospitals: clinics),
      ),
    );
  }
}
...
class _FeaturedClinicsLoading extends StatefulWidget {
  const _FeaturedClinicsLoading();

  @override
  State<_FeaturedClinicsLoading> createState() =>
      _FeaturedClinicsLoadingState();
}

class _FeaturedClinicsLoadingState extends State<_FeaturedClinicsLoading>
