...
class DoctorAddReviewPage extends StatefulWidget {
  const DoctorAddReviewPage({super.key});

  @override
  State<DoctorAddReviewPage> createState() => _DoctorAddReviewPageState();
}

class _DoctorAddReviewPageState extends State<DoctorAddReviewPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<DoctorReviewsCubit, DoctorReviewsState>(
      listenWhen: (p, c) =>
      ...
      listener: (context, state) {
        if (!state.isSubmittingReview && state.submitSuccess) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('doctors.reviews.submit_success'.tr()),
            ),
          );
      ...
      builder: (context, state) {
        ...
        return Scaffold(
          appBar: AppBar(
            title: Text('doctors.reviews.add_title'.tr()),
          ),
          body: SingleChildScrollView(
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'doctors.reviews.rating_label'.tr(),
                  ...
                  if (_rating == 0)
                    Padding(
                      ...
                      child: Text(
                        'doctors.reviews.validation.rating'.tr(),
                  ...
                  Text(
                    'doctors.reviews.comment_label'.tr(),
                    style: theme.textTheme.titleMedium
                        ?.copyWith(fontWeight: FontWeight.w700),
                  ),
                  const SizedBox(height: 8),
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'doctors.reviews.comment_hint'.tr(),
                    ...
                    validator: (value) {
                      if (value == null || value.trim().isEmpty) {
                        return 'doctors.reviews.validation.comment'.tr();
                      }
                      if (_rating == 0) {
                        return 'doctors.reviews.validation.rating'.tr();
                      }
                      return null;
                    },
                  ),
                    ...
                    children: [
                      Expanded(
                        child: OutlinedButton(
                          ...
                          child: Text('doctors.reviews.cancel'.tr()),
                        ),
                      ),
                      ...
                      Expanded(
                        child: ElevatedButton(
                          ...
                          child: isLoading
                              ...
                              : Text('doctors.reviews.submit'.tr()),
                        ),
                      ),
                    ],
                  ),
